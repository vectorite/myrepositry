...
class SalesreporderController extends JController
{
	...
	function get_address()
	{
		...
		//get order previou shipping address
		$st_user_id = '';
		if( isset($_SESSION['orderID']) && ($_SESSION['orderID'] != ""))
		{
		$db->setQuery('SELECT * FROM #__virtuemart_order_userinfos WHERE address_type = "ST" AND  virtuemart_order_id = "'.$_SESSION['orderID'].'" LIMIT 1');
		$st_userdetail = $db->loadObject();		
		
		$db->setQuery('SELECT virtuemart_userinfo_id FROM #__virtuemart_userinfos WHERE  virtuemart_user_id = "'.$_POST['id'].'" AND address_type = "ST" AND  first_name = "'.$st_userdetail->first_name.'" AND last_name="'.$st_userdetail->last_name.'" AND address_1 = "'.$st_userdetail->address_1.'" AND virtuemart_state_id = "'.$st_userdetail->virtuemart_state_id.'" AND zip = "'.$st_userdetail->zip.'" LIMIT 1');
		$st_user_id = $db->loadResult();
				
		}	
		//End code for get order previou shipping address 
		
		$db->setQuery('SELECT state_2_code FROM #__virtuemart_states WHERE  virtuemart_state_id = '.$userdetail->virtuemart_state_id);
		...
		$data[0]=$user_company.$userdetail->title." ".$userdetail->first_name." ".$userdetail->last_name."\n".$userdetail->address_1.", ".$bt_address2.$userdetail->city.", ".$statename."  ".$userdetail->zip."\n".$countryname."\n".$userdetail->phone_1;
		
		$textarea_shipping_address = $userdetail->address_1.", ".$bt_address2.$userdetail->city.", ".$statename."  ".$userdetail->zip."\n".$countryname;
		...
		$html .='<table>
     <tr><td><input onclick="cal_total(\'dataTable\')" type="radio" name="ship_to_info_id" value="'.$userdetail->virtuemart_userinfo_id.'" checked="checked" class="stradio"></td><td colspan="2">Ship to same as bill to address.<input type="hidden" id="shipping_address_'.$userdetail->virtuemart_userinfo_id.'" value="'.$textarea_shipping_address.'"/></td></tr>';
		...
		$db->setQuery('SELECT state_2_code FROM #__virtuemart_states WHERE  virtuemart_state_id = '.$sp_detail->virtuemart_state_id);
		$sp_statename = $db->loadResult();	
		$db->setQuery('SELECT country_name FROM #__virtuemart_countries WHERE  virtuemart_country_id = '.$sp_detail->virtuemart_country_id);
		$sp_countryname = $db->loadResult();
		$chk = '';
		if($_SESSION['edit_select_shipping_id']	== $sp_detail->virtuemart_userinfo_id || $st_user_id 	== $sp_detail->virtuemart_userinfo_id )
		{
		$chk = 'checked="checked"';
		$textarea_shipping_address = $sp_detail->address_1.', '.$sp_address2.$sp_detail->city.', '.$sp_statename.', '.$sp_detail->zip.'\n'.$sp_countryname;
		}
		if($sp_detail->address_type_name != "")
		$html .= '<tr><td valign="top"><input onclick="cal_total(\'dataTable\')" type="radio" name="ship_to_info_id" '.$chk.' value="'.$sp_detail->virtuemart_userinfo_id.'" /></td><td>'.$sp_detail->address_type_name.', '.$sp_detail->address_1.', '.$sp_detail->city.', '.$sp_statename.', '.$sp_detail->zip.', '.$sp_countryname.'<input type="hidden" id="shipping_address_'.$sp_detail->virtuemart_userinfo_id.'" value="'.$sp_detail->address_1.', '.$sp_address2.$sp_detail->city.', '.$sp_statename.', '.$sp_detail->zip.', '.$sp_countryname.'"/></td><td valign="top"><a class="popupwindow" rel="windowCallUnload" href="'.JURI::root().'index.php/register-user?user_type=ST&user_id='.$sp_detail->virtuemart_user_id.'&virtuemart_userinfo_id='.$sp_detail->virtuemart_userinfo_id.'">Edit</a></td></tr>' ;
		else
		$html .= '<tr><td><input '.$chk.' onclick="cal_total(\'dataTable\')" type="radio" name="ship_to_info_id" value="'.$sp_detail->virtuemart_userinfo_id.'" /></td><td>'.$sp_detail->first_name." ".$sp_detail->last_name.', '.$sp_detail->address_1.', '.$sp_detail->city.', '.$sp_statename.', '.$sp_detail->zip.', '.$sp_countryname.'<input type="hidden" id="shipping_address_'.$sp_detail->virtuemart_userinfo_id.'" value="'.$sp_detail->address_1.', '.$sp_address2.$sp_detail->city.', '.$sp_statename.', '.$sp_detail->zip.', '.$sp_countryname.'"/></td><td valign="top"><a class="popupwindow" rel="windowCallUnload" href="'.JURI::root().'index.php/register-user?user_type=ST&user_id='.$sp_detail->virtuemart_user_id.'&virtuemart_userinfo_id='.$sp_detail->virtuemart_userinfo_id.'">Edit</a></td></tr>' ;	
		}
	...
	{
		$db = JFactory::getDBO();
		if($_POST['state_id'] != '10')
		{	
		$sql = 'SELECT COUNT(*) FROM #__virtuemart_zipcode WHERE  zipcode = "'.$_POST['zipcode'].'"';				
		$db->setQuery($sql);
		...
		{	
		$sql = 'SELECT id FROM #__virtuemart_zipcode WHERE virtuemart_state_id="'.$_POST['state_id'].'" AND zipcode = "'.$_POST['zipcode'].'"';				
		$db->setQuery($sql);
	...
	function calculate_tax()
	{
		...
		if($_POST['shipping_amt'] != '' && $_POST['shipping_amt'] != '0.00' && $_POST['shipping_amt'] != '0' )
