...
class WorkforceViewDepartment extends JView
{
    ...
    function display($tpl = null)
	{
		...
		$lists = array();
        $filter_order		= $app->getUserStateFromRequest( $option.'.departments.filter_order', 'filter_order', 'e.ordering', 'cmd' );
		$filter_order_dir	= $app->getUserStateFromRequest( $option.'.departments.filter_order_dir', 'filter_order_dir', 'ASC', 'word' );
        $lists['sort']      = workforceHTML::buildEmployeeSortList($filter_order, 'class="inputbox" onchange="submit()"', 'department');
        ...
        $this->_prepareDocument($department);
    ...
    protected function _prepareDocument($department = null)
    {
		...
		if ($menu) {
			...
			$this->params->def('page_heading', JText::_( 'COM_WORKFORCE_EMPLOYEES' ));
		}

        $title = ($department) ? $department->name : JText::_('COM_WORKFORCE_EMPLOYEES');
        $this->wftitle = $title;
        if (empty($title)) {
