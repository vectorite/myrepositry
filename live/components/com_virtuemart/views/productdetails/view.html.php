...
//		if(!empty($product->related) && is_array($product->related) && !empty($product->related[0]))$product_model->addImages($product->related);
	...
	if($category->category_product_layout != '')
	{
	if($category->category_product_layout != "0")
	$product_page_layout = $category->category_product_layout;
	else
	$product_page_layout = $product->layout;
	}
	else
	{
	$product_page_layout = $product->layout;
	}
	shopFunctionsF::setVmTemplate($this, $category->category_template, $product->product_template, $category->category_layout, $product_page_layout);
