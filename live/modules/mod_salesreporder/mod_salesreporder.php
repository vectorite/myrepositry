...
{
		$query = "SELECT  COUNT(*)  FROM #__virtuemart_orders WHERE order_status='P' AND virtuemart_order_id='".$_GET['orderid']."'";
		$db->setQuery($query);
...
/// select order data if order in pending status.
		...
		$query = 'SELECT `customer_note` FROM `#__virtuemart_orders` WHERE virtuemart_order_id= "'.$_SESSION['orderID'].'"';
		$db->setQuery($query);
		$order_customer_note = $db->loadResult();
		
		
		$query = "SELECT *  FROM #__virtuemart_order_items WHERE order_status='P' AND virtuemart_order_id='".$_SESSION['orderID']."'";
		$db->setQuery($query);
		$_SESSION['pending_orders_products'] = $db->loadAssocList();
		
		if(count($_SESSION['pending_orders_products']) > 0)
		{
		$query0 = "SELECT  order_total,order_salesPrice,order_billTaxAmount,order_shipment  FROM #__virtuemart_orders WHERE order_status='P' AND virtuemart_order_id='".$_SESSION['orderID']."'";
		$db->setQuery($query0);
		...
		$query = "SELECT  virtuemart_user_id  FROM #__virtuemart_orders WHERE order_status='P' AND virtuemart_order_id='".$_SESSION['orderID']."'";
		$db->setQuery($query);
...
var rowCount1 = 1;
<?php if(count($_SESSION['pending_orders_products']) > 0){ ?> rowCount1 = <?php echo  count($_SESSION['pending_orders_products']) ?>;  <?php } ?>
function get_user_address(id) { 
  ...
  $.ajax({  
			...
			//$("#edit_user_info_link").html("<a href='index.php/register-user?user_type=BT&user_id="+id+"'>Edit Customer Info.</a>");
		...
		function cal_total(tableID)
			...
			//data: { 'user_id': user_id ,'vm_user_id': vm_user_id ,'total_amt':(total_amt+total_shipping)-discount_amt,'discount_amt':discount_amt,'shipping_amt':total_shipping},   				
				...
				success: function(responce){
					...
					if(discount_amt > 0 && total_amt>0)alert("Discount amount greter then to total amount.");
					}
...
<?php if($_SESSION['pre_user_id_sess']!="" && $_SESSION['pre_user_id_sess']!="0"){ ?>
$(document).ready(function() {
 	 	var value1 = '<?php echo $_SESSION['pre_user_id_sess']; ?>';
		if(value1 > 0){
...
<?php }?>
	...
	//alert("second_test"+t);
	window.location.href = t+"?orderid="+order_id;	
	}
	}*/
	...
	function selectuser()
	{
	var user_id = document.getElementById("select_user_id").value;
	$(".popupwindow").popupwindow(profiles);
...
<table width="523" border="0" cellspacing="10px">
  <tr>
   ...
   <tr>
        <td width="173"><span style="margin-right:200px;float:right;"><strong>Add New Customer:</strong><a href="index.php/register-user" onclick="clear_pending_order();">Click here</a></span></td>
  </tr> 
  <tr>      
        <td >
        ...
        <?php foreach($salesRepsList as $salesReps){
  ...
  <tr>      
        ...
        <select <?php if($_SESSION['pre_user_id_sess'] != ""){ echo 'disabled="disabled"'; }?> name="customer_name1" id="customer_name1" onchange="get_user_address(this.value)" >
        <option value="">--Select--</option>
        <?php foreach($usersList as $userdetail){
		if($_SESSION['customer_id_session']==$userdetail->id)$sel_user = 'selected="selected"';
		 ?>
        <option <?php echo $sel_user; ?>  value="<?php echo $userdetail->id; ?>"><?php echo $userdetail->email; ?></option>
        <?php $sel_user=''; } ?>
        </select>&nbsp;&nbsp;<span id='edit_user_info_link'></span>
     ...
     <tr>
     <td valign="top"><span style="width:315px;float:left;"><textarea style="width:300px;height:110px;" id="user_address_detail" name="user_address_detail" readonly="readonly"><?php echo $_SESSION['customer_addr_session']; ?></textarea></span>
   ...
   <tr>
     ...
     <table id="dataTable">
     <?php if(count($_SESSION['pending_orders_products']) > 0){
	  $w1 = 0;
	 foreach($_SESSION['pending_orders_products'] as $cart_sess_product)
	 {
     ...
     <td valign="top"><input type="text" onblur="cal_total('dataTable')" onkeyup="cal_total('dataTable')" value="<?php echo $cart_sess_product['qb_discount']; ?>" name="item_discout[]" class="item_rate" id="item_discout_1<?php echo $w; ?>" style="width:80px;"></td>     
     </tr>
     ...
     <td valign="top"><input type="text" onkeyup="cal_total('dataTable')" value="0.00" name="item_discout[]" class="item_rate" id="item_discout_1" style="width:80px;" onblur="cal_total('dataTable')"></td>   
     </tr>
     <?php } ?>
     </table>     
     </td>     
     ...
     <tr>
     <td valign="top" align="right"><strong>Sub Total:</strong><input readonly="readonly" type="text" name="item_sub_total" id="item_sub_total" onfocus="cal_total('dataTable')" value="0.00" /></br>
     <strong>Discount Total:</strong> <input readonly="readonly" type="text" name="item_sub_discount" id="item_sub_discount" onkeyup="cal_total('dataTable')"  value="<?php if($pending_orders_discount != '')echo number_format($pending_orders_discount,2, '.', ''); else echo '0.00'; ?>"/></br>
     <strong>Tax:</strong> <input readonly="readonly" type="text" name="item_total_tax" id="item_total_tax" onfocus="cal_total('dataTable')" value="0.00" /></br>
     <strong>Shipping:</strong> <input type="text" name="item_total_shipping" id="item_total_shipping"  onblur="cal_total('dataTable')"  value="<?php if($pending_orders_shipping != '')echo number_format($pending_orders_shipping,2, '.', ''); else echo '0.00'; ?>" /></br>
      ...
      <td>
                ...
                <table>
                    ...
                    <td>
                        ...
                        <input type="text" id="CardNumber"  name="cc_card_number" style="width:195px" value="<?php echo $_SESSION["cc_card_number"]; ?>">
                    </td>
                   </tr>
                    <tr valign="top">
                    <td>Expiry date</td>
		        	<td> <select id="cc_expire_month_2" name="cc_expire_month">
                        ...
                        <option value="01" <?php if($_SESSION["cc_expire_month_2"] == '01') echo 'selected="selected"'; ?>>January</option>
                        <option value="02" <?php if($_SESSION["cc_expire_month_2"] == '02') echo 'selected="selected"'; ?>>February</option>
                        <option value="03" <?php if($_SESSION["cc_expire_month_2"] == '03') echo 'selected="selected"'; ?>>March</option>
                        <option value="04" <?php if($_SESSION["cc_expire_month_2"] == '04') echo 'selected="selected"'; ?>>April</option>
                        <option value="05" <?php if($_SESSION["cc_expire_month_2"] == '05') echo 'selected="selected"'; ?>>May</option>
                        <option value="06" <?php if($_SESSION["cc_expire_month_2"] == '06') echo 'selected="selected"'; ?>>June</option>
                        <option value="07" <?php if($_SESSION["cc_expire_month_2"] == '07') echo 'selected="selected"'; ?>>July</option>
                        <option value="08" <?php if($_SESSION["cc_expire_month_2"] == '08') echo 'selected="selected"'; ?>>August</option>
                        <option value="09" <?php if($_SESSION["cc_expire_month_2"] == '09') echo 'selected="selected"'; ?>>September</option>
                        <option value="10" <?php if($_SESSION["cc_expire_month_2"] == '10') echo 'selected="selected"'; ?>>October</option>
                        <option value="11" <?php if($_SESSION["cc_expire_month_2"] == '11') echo 'selected="selected"'; ?>>November</option>
                        <option value="12" <?php if($_SESSION["cc_expire_month_2"] == '12') echo 'selected="selected"'; ?>>December</option>
                    </select>
                     / <select id="cc_expire_year_2" name="cc_expire_year">
                        <option value="0">YEAR</option>
                        <option value="2012" <?php if($_SESSION["cc_expire_year_2"] == '2012') echo 'selected="selected"'; ?>>2012</option>
                        <option value="2013" <?php if($_SESSION["cc_expire_year_2"] == '2013') echo 'selected="selected"'; ?>>2013</option>
                        <option value="2014" <?php if($_SESSION["cc_expire_year_2"] == '2014') echo 'selected="selected"'; ?>>2014</option>
                        <option value="2015" <?php if($_SESSION["cc_expire_year_2"] == '2015') echo 'selected="selected"'; ?>>2015</option>
                        <option value="2016" <?php if($_SESSION["cc_expire_year_2"] == '2016') echo 'selected="selected"'; ?>>2016</option>
                        <option value="2017" <?php if($_SESSION["cc_expire_year_2"] == '2017') echo 'selected="selected"'; ?>>2017</option>
                        <option value="2018" <?php if($_SESSION["cc_expire_year_2"] == '2018') echo 'selected="selected"'; ?>>2018</option>
                        <option value="2019" <?php if($_SESSION["cc_expire_year_2"] == '2019') echo 'selected="selected"'; ?>>2019</option>
                        <option value="2020" <?php if($_SESSION["cc_expire_year_2"] == '2020') echo 'selected="selected"'; ?>>2020</option>
                    </select>
					</td>  </tr>
                          ...
                          <tr>
                    		<td>
                                ...
                                <input type="text" maxlength="4" id="cvv" name="cc_cvv" style="width: 105px;" value="<?php echo $_SESSION["cc_cvv"]; ?>">
                            </td>
                    	 </tr>
                ...
                <strong>Note:</strong>
                <table><tr><td>
                <textarea name="sales_note" style="width: 300px; height: 85px;"><?php if($order_customer_note != "") echo $order_customer_note; else  echo $_POST['sales_note']; ?></textarea>
                </td></tr></table>
                </span>         
      </td>
      </tr>	
