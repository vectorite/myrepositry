...
{
	function includeAssets($loadJQuery, $noConflict, $jQueryVer = '1.4.4', $options = array(), $params = null)
	{
		static $loaded;
		...
		if ($loadJQuery) 
		{
			...
			{
				AriDocumentHelper::includeJsFile($baseUri . 'jquery.min.js');
			}
			else
			{
	...
	function activateAutoShow($group = '', $autoShow = true, $uniqueKey = false, $cookie = null, $delay = 0, $autoCloseDelay = 0)
	{
		if ($autoShow != 'first')
		...
		$document =& JFactory::getDocument();
		$document->addScriptDeclaration(sprintf(';(window["jQueryASL"] || jQuery)(document).ready(function($){ setTimeout(function() { $("a[rel^=\'%1$s\']").eq(0).click(); },%2$d);%3$s });',
			$group 
				? 'sexylightbox[' . $group . ']'
				: 'sexylightbox',
			$delay > 0 ? 1000 * $delay : 0,
			$autoCloseDelay > 0 
				? sprintf('setTimeout(function() {SexyLightbox.close();}, %1$d);',
					1000 * $autoCloseDelay + ($delay > 0 ? 1000 * $delay : 0))
				: '' 
			)
		);
