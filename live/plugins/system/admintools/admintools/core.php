...
class plgSystemAdmintoolsCore extends JPlugin
{
	...
	private function jscombine()
	{
	 	...
	 	foreach( $matches[1] as $url )
		{
			...
			if($this->isInternal($file))
			{
				...
				if( !@is_readable(JPATH_ROOT.DS.$file) ) {
					$replace = false;
				}
		...
		if(!JFile::exists($this->combineCache.DS.'js-'.$jsHash.'.js') || self::DEBUG)
		{
			...
			JFile::write($this->combineCache.DS.'js-'.$jsHash.'.js', $jsContent);
		}
	...
	private function csscombine()
	{
		...
		// Does the cache file exist?
		if(!JFile::exists($this->combineCache.DS.'css-'.$cssHash.'.css') || self::DEBUG)
		{
			// Nope. Let's create the file.
			$hashable = '';
			$cssContent = "";
			foreach($this->cssFiles as $file)
			{
				...
				if(DS == '\\') $basePath = str_replace (DS, '/', $basePath);
			...
			// Write the cache file
			JFile::write($this->combineCache.DS.'css-'.$cssHash.'.css', $cssContent);
		}
	...
	private function deliverFile($hash)
	{
		...
		// Is the file there?
		if($pass) $pass = $pass && JFile::exists($this->combineCache.DS.$hash.$ext);
		
		// Can we read the file?
		if($pass)
		{
			$content = JFile::read($this->combineCache.DS.$hash.$ext);
			if($content === false) {
		...
		jimport('joomla.utilities.date');
		$date = new JDate();
		$filedate = @filemtime($this->combineCache.DS.$hash.$ext);
