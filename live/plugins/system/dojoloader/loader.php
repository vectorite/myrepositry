...
class DojoLoader{
  ...
  function _build(){
    ...
    $hash = md5(implode('', $keys).implode('', $fkeys).$script).'.js';
    
    $folder = $this->checkFolders();
    
    $path = JPATH_SITE.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'dojo'.DIRECTORY_SEPARATOR.$folder.DIRECTORY_SEPARATOR.$hash;
