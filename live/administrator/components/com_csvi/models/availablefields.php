 ...
 * @version 	$Id: availablefields.php 2048 2012-07-28 16:27:43Z RolandD $
 */
			...
			switch ($component->value) {
				case 'com_virtuemart':
					...
					$query = "INSERT IGNORE INTO ".$db->quoteName('#__csvi_available_fields')."(csvi_name, component_name, component_table, component) 
							(SELECT TRIM(custom_title), TRIM(custom_title), 'productimport', 'com_virtuemart' FROM #__virtuemart_customs WHERE field_type IN ('S', 'I', 'B', 'D', 'T', 'M'))";
					$db->setQuery($query);
					$db->query();
					
					// Add custom fields as available field to product import
					$query = "INSERT IGNORE INTO ".$db->quoteName('#__csvi_available_fields')."(csvi_name, component_name, component_table, component)
					(SELECT TRIM(custom_title), TRIM(custom_title), 'productexport', 'com_virtuemart' FROM #__virtuemart_customs WHERE field_type IN ('S', 'I', 'B', 'D', 'T', 'M'))";
					$db->setQuery($query);
					$db->query();
					break;
			}
