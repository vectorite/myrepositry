...
class vmJsApi{
	...
	static function JcountryZipcodeList($zipcodeIds) {
	
		static $JcountryZipcodeList;
		// If exist exit
		if ($JcountryZipcodeList) {
			return;
		}
		$document = JFactory::getDocument();
		VmJsApi::jSite();
		$document->addScriptDeclaration(' jQuery( function($) {
			$("select.virtuemart_state_id").vm2front("list",{dest : "#virtuemart_zipcode_id",ids : "'.$zipcodeIds.'"});
		});');
		$JcountryZipcodeList = TRUE;
