...
class VirtuemartViewProduct extends JView {
	...
	function display($tpl = null) {
		...
		$this->type = JRequest::getVar('type', false);
		...
		else if ($this->type=='relatedproducts_1') {
			 $query = "SELECT virtuemart_product_id AS id, CONCAT(product_name, '::', product_sku) AS value
				FROM #__virtuemart_products_".VMLANG."
				 JOIN `#__virtuemart_products` AS p using (`virtuemart_product_id`)";
			if ($filter) $query .= " WHERE product_name LIKE '%". $this->db->getEscaped( $filter, true ) ."%' or product_sku LIKE '%". $this->db->getEscaped( $filter, true ) ."%' limit 0,10";
			self::setRelatedHtml($query,'W');
		...
		else if ($this->type=='relatedcategories_1')
		{
			$query = "SELECT virtuemart_category_id AS id, CONCAT(category_name, '::', virtuemart_category_id) AS value
				FROM #__virtuemart_categories_".VMLANG;
			if ($filter) $query .= " WHERE category_name LIKE '%". $this->db->getEscaped( $filter, true ) ."%' limit 0,10";
		
			self::setRelatedHtml($query,'O');
		}
		//code for articles 
		else if ($this->type=='relatedarticles_1')
		{
			$query = "SELECT  id , title,images FROM #__content WHERE title LIKE '". $this->db->getEscaped( $filter, true ) ."%' limit 0,10";
			$this->db->setQuery($query);
			if($_SESSION['ww'] == '')			
			$_SESSION['ww']=1;			
			foreach ($this->db->loadObjectList() as $related) {
			$ra->id = $related->id;
			$ra->value = $related->title;
			$query = 'SELECT * FROM `#__virtuemart_customs` WHERE field_type ="J" ';
			$this->db->setQuery($query);
			$customs = $this->db->loadObject();
			$img_src = json_decode($related->images);
			...
			$display = '<span><input value="'.$related->id.'" name="field['.($this->row+$_SESSION['ww']).'][custom_value]" type="hidden"><a href="'.JURI::root().'index.php?option=com_content&view=article&id='.$related->id.'" title="'.$related->title.'"><img src="'.JURI::root().'/'.$img_path.'" >'.$related->title.'</a></span>';
			
			$html = '<div class="vm_thumb_image">
				<span>'.$display.'</span>	
				<input value="'.$customs->field_type.'" name="field['.($this->row+$_SESSION['ww']).'][field_type]" type="hidden">
				<input value="'.$customs->virtuemart_custom_id.'" name="field['.($this->row+$_SESSION['ww']).'][virtuemart_custom_id]" type="hidden">
				<input value="0" name="field['.($this->row+$_SESSION['ww']).'][virtuemart_customfield_id]" type="hidden">
				<input value="0" checked="checked" name="field['.($this->row+$_SESSION['ww']).'][admin_only]" type="hidden">			
				<div class="vmicon vmicon-16-remove"></div></div>';
			 $ra->label = $html;
			 $ra1[] = $ra;
			 
		}$_SESSION['ww'] = $_SESSION['ww']+1;	
		echo json_encode($ra1);	die; 
		
		}
		
		
		else if ($this->type=='relatedarticles_2')
		{
			$query = "SELECT  id , title,images FROM #__content WHERE title LIKE '". $this->db->getEscaped( $filter, true ) ."%' limit 0,10";
			$this->db->setQuery($query);
			if($_SESSION['ww'] == '')			
			...
			$ra->id = $related->id;
			$ra->value = $related->title;
			$query = 'SELECT * FROM `#__virtuemart_customs` WHERE field_type ="K"';
			$this->db->setQuery($query);
			$customs = $this->db->loadObject();
			...
			$html = '<div class="vm_thumb_image">
				<span>'.$display.'</span>	
				<input value="'.$customs->field_type.'" name="field['.($this->row+$_SESSION['ww']).'][field_type]" type="hidden">		<input value="'.$customs->virtuemart_custom_id.'" name="field['.($this->row+$_SESSION['ww']).'][virtuemart_custom_id]" type="hidden"><input value="0" name="field['.($this->row+$_SESSION['ww']).'][virtuemart_customfield_id]" type="hidden"><input value="0" checked="checked" name="field['.($this->row+$_SESSION['ww']).'][admin_only]" type="hidden"><div class="vmicon vmicon-16-remove"></div></div>';
			 $ra->label = $html;
			 $ra1[] = $ra;
