 ...
 * @author Max Milbers
 */
class VirtuemartViewZipcode extends VmView {
...
//		$model->setId($stateId);
		...
		$countryId = JRequest::getInt('virtuemart_state_id', 0);
		if(empty($countryId)) $countryId = $state->virtuemart_country_id;
		$this->assignRef('virtuemart_state_id',	$countryId);

        $isNew = (count($state) < 1);

		if(empty($countryId) && $isNew){
			JError::raiseWarning(412,'State id is 0');
			return false;
		}

		$country = VmModel::getModel('state');
		$country->setId($countryId);
		$this->assignRef('state_name', $state->getData()->state_name);
		...
		if ($layoutName == 'edit') {
			...
			$zipcodes = $model->getZipcodes($stateId);
			$this->assignRef('zipcodes',	$zipcodes);
