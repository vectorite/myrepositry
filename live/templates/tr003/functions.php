...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    function artxPost($data)
    {
    	...
    	artxFragmentBegin(str_replace('class="nim-box nim-post">', 'class="nim-box nim-post' . $classes . '">', "<div class=\"nim-box nim-post\">\r\n    <div class=\"nim-box-body nim-post-body\">\r\n<div class=\"nim-post-inner\">\r\n"));
    	artxFragmentBegin("<h2 class=\"nim-postheader\">");
    	artxFragmentBegin("");
...
?>
        <?php ob_start(); ?>
<div class="nim-box nim-block">
            <div class="nim-box-body nim-block-body">
        
        <?php echo str_replace('class="nim-box nim-block">', 'class="nim-box nim-block' . $classes . '">', ob_get_clean()); ?>
        <?php if ($hasCaption): ?>
<div class="nim-bar nim-blockheader">
            <h3 class="t">
        <?php echo $caption; ?>
</h3>
        </div>
        <?php endif; ?>
        <?php if ($hasContent): ?>
<div class="nim-box nim-blockcontent">
            <div class="nim-box-body nim-blockcontent-body">
...
<?php
    ...
    function artxModules(&$document, $position, $style = null)
    {
         ...
         *  Select inputs with class != 'nim-button'
         *   artxFindTags($html, array('input' => array('class' => '!nim-button')))
         *  Select inputs with non-empty class
         *   artxFindTags($html, array('input' => array('class' => '!')))
         *  Select inputs with class != 'nim-button' and non-empty class:
         *   artxFindTags($html, array('input' => array('class' => array('!nim-button', '!'))))
         *  Select inputs with class = button but != 'nim-button'
         *   artxFindTags($html, array('input' => array('class' => array('button', '!nim-button'))))
         *
        ...
        function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
        {
            ...
            foreach (artxFindTags($content, $filter) as $tag) {
                if (is_string($tag))
                    ...
                    $tag['attributes']['class']['value'][] = 'nim-button';
                    $delimeter = '' == $tag['attributes']['class']['delimeter']
                        ? '"' : $tag['attributes']['class']['delimeter'];
                    $tag['attributes']['class']['delimeter'] = $delimeter;
                    $button = str_replace('"', $delimeter, '<span class="nim-button-wrapper">'
                        . '<span class="nim-button-l"> </span><span class="nim-button-r"> </span>')
        ...
        {
            return '<span class="nim-button-wrapper"><span class="nim-button-l"> </span><span class="nim-button-r"> </span>'
                . '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
                . 'nim-button" href="' . $data['link'] . '">' . $data['content'] . '</a></span>';
        }
...
function artxPostprocessBlockContent($content)
    ...
    {
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!nim-button')),
            'button' => array('class' => array('button', '!nim-button'))));
        return $content;
    }
