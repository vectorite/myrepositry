...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="nim-content-layout">';
        $result .= '<div class="nim-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="nim-layout-cell' . ('nim-block' == $style ? ' nim-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="nim-post">' string in it.
     * Then it wraps content of the buffer with nim-post.
     */
    public function componentWrapper()
    {
        ...
        if ('com_content' == $option && ('featured' == $view || 'article' == $view || ('category' == $view && 'blog' == $layout)))
            return;
        if (false === strpos($content, '<div class="nim-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'nim-main').addClass('nim-postcontent');
            }
        });
    }
});
