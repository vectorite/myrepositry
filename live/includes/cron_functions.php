 ...
 */
        ...
        $db =  mysql_connect('localhost', 'db1_tileredi', 'MyCat8aRabbit'); 
	$conn2 = mysql_connect('192.168.20.218', 'live_sales', 'RHfGeMcD5pY2VzN7', true); 
	$conn3 = mysql_connect('192.168.20.218', 'live_usa', 'UTqK5jYqSRuFqyrS', true);
        $salesDB = 'live_sales';
        $usaDB = 'live_usa';
        $vmDB = 'db1_tileredi';
        $debug = "0";
...

{
  ...
  if ($country == 'Canada')
        {
        $link = mysql_connect('192.168.20.218', 'live_usa', 'UTqK5jYqSRuFqyrS', true);
        $database_name = 'live_usa';
        }
        else 
        {
        $link = mysql_connect('192.168.20.218', 'live_sales', 'RHfGeMcD5pY2VzN7',true);
        $database_name = 'live_sales';
}

//echo "<br>".$database_name." ".$link."<br>";
        
  mysql_select_db($database_name,$link);
  $query_queryLookup = "SELECT Name, Email FROM customer WHERE Email = '$email_address'";
