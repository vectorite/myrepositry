...
class JElementSkin extends JOfflajnFakeElementBase
{
    ...
    $script = '
      ...
      function ffireEvent(element,event){
        if ((document.createEventObject && !dojo.isIE) || document.createEventObject && dojo.isIE < 9){
        // dispatch for IE
      ...
      function changeSkins'.$orig_name.'(el){
        if("'.$orig_name.'" == "themeskin" && window.themelevel && window.themelevel.levels.length<4){
