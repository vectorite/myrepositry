...
dojo.declare("FontConfigurator", null, {
  ...
  reset: function(){
    ...
    if(this.hidden.value != dojo.toJson(this.settings)){
      var newsettings = dojo.fromJson(this.hidden.value);
  ...
  showWindow: function(e){
    ...
    if(!this.window){
      ...
      this.settings = dojo.fromJson(this.hidden.value);
      this.loadSettings();
    }
  ...
  loadGoogleFont: function(subset, family, weight, italic){
    ...
    if(!window.loadedFont[hash]){
      ...
      setTimeout(function(){
        dojo.create('link', {rel:'stylesheet', type: 'text/css', href: 'https://fonts.googleapis.com/css?family='+family+':'+weight+italic+'&subset='+subset}, dojo.body())
      },500);
    } 
  },
