...
class modsalesrepOrderHelper
{
	...
	function sendEmail($params)
	{
		...
		$add_details = $_POST['user_address_detail'];
		$item_tax    = $_POST['item_total_tax'];
		...
		$_SESSION['item_discout_total'] = $_POST['item_total_tax'];*/
		...
		//$session->set('current_user_country_id',$current_user_country_id);
		...
		$item_discout = $_POST['item_discout'];
		...
		$discout_sub_total = (float)number_format($_POST['item_sub_discount'], 2, '.', '');
		
		$prices['salesPrice'] = $_POST['item_sub_total'];
		$prices['withTax'] = $_POST['item_total_amount'];
		$prices['billTotal'] = $_POST['item_total_amount'];
		
		$cart123->pricesUnformatted['billTotal'] = $_POST['item_total_amount'] + $discout_sub_total;
		$cart123->pricesUnformatted['withTax'] = $_POST['item_total_amount'];
		$cart123->pricesUnformatted['discountAfterTax'] = $_POST['item_total_amount'];
	
		
		///add shipping price in total prices
		if($_POST['item_total_shipping'] != "0.00")
		{
		$prices['shipmentValue'] = $_POST['item_total_shipping'];
		$prices['shipmentTotal'] = $_POST['item_total_shipping'];		
		}
...
/////////============= payment by authorize.net ================/////////
		...
		if($prices['billTotal'] > 0)
		{
			...
			{
			$x_login = "8Vu75v8U6";
			$x_tran_key = "8Uq22Ue3zvtK9P6a";
			}
			else
			{
			$x_login = "6am2JHY95Dv";
			$x_tran_key = "4B85434pY5u6Vrxg";
			}
...
/////////============= Code END payment by authorize.net ================/////////				
	...
	$jAp->redirect(JURI::root().'index.php/sales-orders/', "Order inserted Successfully. Order Number is --> ".$current_order_number);
	}
		else
		{
			$_SESSION["cc_card_number"]		= $_POST['cc_card_number'];
			$_SESSION["cc_expire_month_2"]	= $_POST['cc_expire_month'];
			$_SESSION["cc_expire_year_2"]		= $_POST['cc_expire_year'];
		...
		$_SESSION['orderID'] = $orderID;
		$cart123->emptyCart();
		$jAp->redirect(JURI::root().'index.php/sales-orders/', "Order Save Successfully. Order Number is --> ".$current_order_number);
		}
		else
		{
