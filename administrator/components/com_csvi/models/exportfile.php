 ...
 * @version 	$Id: exportfile.php 2061 2012-08-07 09:20:26Z RolandD $
 */
...
class CsviModelExportfile extends JModel {
	...
	private function _exportDetails() {
		...
		switch ($exportto) {
			case 'tofile':
				...
				break;
			case 'toftp':
				$csvilog->addDebug(JText::sprintf('COM_CSVI_DEBUG_EXPORT_FTP_HOST', $template->get('ftphost', 'general', '', 'string')));
				$csvilog->addDebug(JText::sprintf('COM_CSVI_DEBUG_EXPORT_FTP_PORT', $template->get('ftpport', 'general')));
				$csvilog->addDebug(JText::sprintf('COM_CSVI_DEBUG_EXPORT_FTP_USERNAME', $template->get('ftpusername', 'general', '', 'string')));
				$csvilog->addDebug(JText::sprintf('COM_CSVI_DEBUG_EXPORT_FTP_ROOT', $template->get('ftproot', 'general', '', 'string')));
				break;
		}
	...
	protected function addExportFields($record) {
		...
		foreach ($export_fields as $column_id => $field) {
			if ($field->process && !$template->isCombine($field->field_id)) {
				// Get the value
				$fieldvalue = $record->output[$field->field_id];
					
				// See if we need to combine
				if (is_array($field->combine) && !empty($field->combine)) {
					foreach ($field->combine as $combine_id) {
						$combine_name = $export_fields[$combine_id]->field_name;
							
						if (isset($record->output[$combine_name])) $fieldvalue .= $field->combine_char.$record->output[$combine_name];
					}
				}
