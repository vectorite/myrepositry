 ...
 * @version 	$Id: productimport.php 2059 2012-08-04 13:36:46Z RolandD $
 */
...
class CsviModelProductimport extends CsviModelImportfile {
	...
	private function _loadCustomFields() {
		...
		$query->select("virtuemart_custom_id AS id, field_type, LOWER(TRIM(custom_title)) AS title");
							...
							if (substr($file_details['name'], 0, 4) == 'http') {
								...
								if ($file_details['isimage']) {
									...
									if ($template->get('thumb_create', 'image')) {
										...
										if (empty($thumb) || $generate_image) $thumb = 'resized/'.basename($media['file_url']);
	...
	private function _processCustomAvailableFields() {
		...
		foreach ($this->_customfields as $field) {
			...
			if (isset($this->$title)) {
				...
				$query->delete('#__virtuemart_product_customfields');
				$query->where('virtuemart_product_id = '.$this->virtuemart_product_id);
				$query->where('virtuemart_custom_id = '.$field->id);
				$db->setQuery($query);
				$db->query();
				...
				switch ($field->field_type) {
					...
					case 'M':
						...
						$query->where('file_url = '.$db->quote($this->$title));
						$db->setQuery($query);
						$value = $db->loadResult();
						break;
					default:
				...
				$query->values($this->virtuemart_product_id.', '.$field->id.','.$db->quote($value));
				$db->setQuery($query);
				$db->query();
