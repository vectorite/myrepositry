...
// 			}
	 ...
	 */
	public function _createOrder($_cart, $_usr, $_prices)
	{
	
		/*echo "<pre>";print_r($_cart);
		echo "-------------------------------\n";
		print_r($_usr);
		echo "-------------------------------\n";
		print_r($_prices);
...
// 		return $this->getCurrencyIsoCode($vendorCurrency);
	 ...
	 */
	public function _writeUserInfo($_id, &$_usr, $_cart)
	{
		$_userInfoData = array();
...
// 			$_userInfoData['virtuemart_order_userinfo_id'] = null; // Reset key to make sure it doesn't get overwritten by ST
	 ...
	 * @return boolean True on success
	 */
	public function _createOrderLines($_id, $_cart)
	{
...
/**
	 ...
	 * @return boolean True on success
	 */
	public function _createOrderCalcRules($order_id, $_cart)
	{
...
// 			}
        $CartData = $_cart->cartData[$calculation_kind];
		    foreach($CartData as $rule){
