...
// 					$options[] = array( 'value' => 'product_name' ,'text' =>JText::_('COM_VIRTUEMART_PRODUCT_FORM_NAME')); Is anyway displayed there
				...
				case 'M':
					...
					$q = 'SELECT `virtuemart_media_id` as value,`file_title` as text FROM `#__virtuemart_medias` WHERE `published`=1 AND (`virtuemart_vendor_id`= "' . $vendorId . '" OR `shared` = "1")';
					$this->_db->setQuery ($q);
				...
				case 'Q':
					...
					$q = 'SELECT `virtuemart_product_id` as value,`product_sku` as text FROM `#__virtuemart_products` WHERE `published`=1';
					$this->_db->setQuery ($q);
					$options = $this->_db->loadObjectList ();
					return JHTML::_ ('select.genericlist', $options, 'field[' . $row . '][custom_value]', '', 'value', 'text', $field->custom_value) . '</td><td>' . $priceInput;
					break;	
	...
	public function getProductCustomsFieldRelatedCategories ($product) {
		...
		$query = 'SELECT C.`virtuemart_custom_id` , `custom_parent_id` , `admin_only` , `custom_title` , `custom_tip` , C.`custom_value` AS value, `custom_field_desc` , `field_type` , `is_list` , `is_hidden` , C.`published` , field.`virtuemart_customfield_id` , field.`custom_value`, field.`custom_param`, field.`custom_price`, field.`ordering`
			FROM `#__virtuemart_customs` AS C
			LEFT JOIN `#__virtuemart_product_customfields` AS field ON C.`virtuemart_custom_id` = field.`virtuemart_custom_id`
			Where `virtuemart_product_id` =' . (int)$product->virtuemart_product_id . ' and `field_type` IN ("Z","K")';
		...
		if ($productCustoms = $this->_db->loadObjectList ()) {
			...
			foreach ($productCustoms as & $field) {
				...
				if($field->field_type == 'K')
				{				
				$field->display .=  '
							 <span>';
							$db = JFactory::getDBO();
							$query = "SELECT  id , title,images FROM #__content WHERE id ='".$field->custom_value."' limit 1";
							$db->setQuery($query);
							$ca = $db->loadObject();
							...
							$img_path = 'images/noimage.jpg';
                                                        ...
                                                        if($img_src->image_intro_caption != '')
							{
							 $img_cap = $img_src->image_intro_caption;
							}
							else
							{ 
							$img_cap = $img_src->image_intro_caption;
							}
                                                        if($img_src->image_intro_alt != '')
							{
							 $img_url = $img_src->image_intro_alt;
							}
							else
							{ 
							$img_url = 'index.php?option=com_content&view=article&id='.$ca->id;
							}
                                                        
						$field->display .='<a href="'.JURI::root().$img_url.'" title="'.$img_cap.'"><img src="'.JURI::root()."/".$img_path.'" >'.$img_cap.'</a></span>';
				
				}
				else //code for article end
				{
				$field->display = $this->displayProductCustomfieldFE($field, $row);
				}
				$row++;
	...
	public function getProductCustomsFieldRelatedProductsCategories($product) {

		$query = 'SELECT C.`virtuemart_custom_id` , `custom_parent_id` , `admin_only` , `custom_title` , `custom_tip` , C.`custom_value` AS value, `custom_field_desc` , `field_type` , `is_list` , `is_hidden` , C.`published` , field.`virtuemart_customfield_id` , field.`custom_value`, field.`custom_param`, field.`custom_price`, field.`ordering`,C.`field_type`
			FROM `#__virtuemart_customs` AS C
			LEFT JOIN `#__virtuemart_product_customfields` AS field ON C.`virtuemart_custom_id` = field.`virtuemart_custom_id`
			Where `virtuemart_product_id` =' . (int)$product->virtuemart_product_id . ' and `field_type` IN ("W","O","J")';
		...
		if ($productCustoms = $this->_db->loadObjectList ()) {
			...
			foreach ($productCustoms as & $field) {
				...
				$field->display .='<span><a href="'.JURI::root().$img_url.'" title="'.'$img_cap'.'"><img src="'.JURI::root()."/".$img_path.'" >'.$img_cap.'</a></span>';			
						...
						if ($group->field_type == 'U') {
							...
							if ($group->field_type == 'A') {
								...
								foreach ($group->options as $productCustom) {
									...
									$productCustom->field_type = $group->field_type;
									$productCustom->is_cart = 1;
								//	$group->display .= '<input id="' . $productCustom->virtuemart_custom_id . '" ' . $checked . ' type="radio" value="' . $productCustom->virtuemart_custom_id . '" name="customPrice[' . $row . '][' . $productCustom->virtuemart_customfield_id . ']" /><label
								//		for="' . $productCustom->virtuemart_custom_id . '">' . $this->displayProductCustomfieldFE ($productCustom, $row) . ' ' . $price . '</label>';
						//MarkerVarMods
						
							/*if($group->virtuemart_custom_id == 47)
							{
							if($ww > 1)$checked ='';
							$group->display .= '<div class="prod_attr_main"><div class="prod_attr_radio"><input id="'. $productCustom->virtuemart_custom_id . '" ' . $checked . ' type="radio" value="' . $productCustom->virtuemart_customfield_id . '" name="customPrice[' . $product->virtuemart_product_id . '][' . $productCustom->virtuemart_custom_id . ']" /></div><div class="prod_attr_img"  for="' . $productCustom->virtuemart_custom_id . '">' . $this->displayProductCustomfieldFE ($productCustom, $row) . ' </div><div class="prod_attr_text">' . $price . '</div></div>';
							
							}*/
							 
							if($group->custom_title == 'Choose Tileable Top or Designer Grate Below.' && $group->virtuemart_custom_id == '47')
							{
							if($ww > 1)$checked ='';
							$db = JFactory::getDBO();
							//print_r($productCustom);die;
							$q = 'SELECT `product_sku` FROM `#__virtuemart_products` WHERE `virtuemart_product_id`= "' . (int)$productCustom->custom_value . '"';
							$db->setQuery ($q);
						    $product_sku = $db->loadResult ();
					
							$group->display .= '<div class="prod_attr_main"><div class="prod_attr_radio"><input id="'. $productCustom->virtuemart_custom_id . '"   type="radio" value="' . $productCustom->virtuemart_customfield_id . '" name="customPrice[' . $product->virtuemart_product_id . '][' . $productCustom->virtuemart_custom_id . ']" /></div><div class="prod_attr_img"  for="' . $productCustom->virtuemart_custom_id . '">' . $this->displayProductCustomfieldFE ($productCustom, $row) . ' </div><div class="prod_attr_text">' . "Model:</br>".$product_sku."</br>".$price . '</div></div>';
							
							}
							else
							{
							
							$group->display .= '<input id="' . $productCustom->virtuemart_custom_id . '" ' . $checked . ' type="radio" value="' . $productCustom->virtuemart_customfield_id . '" name="customPrice[' . $row . '][' . $productCustom->virtuemart_custom_id . ']" /><label for="' . $productCustom->virtuemart_custom_id . '">' . $this->displayProductCustomfieldFE ($productCustom, $row) . ' ' . $price . '</label>';
							}		
		...
		if ($is_list > 0) {
			...
			switch ($type) {
				...
				case 'Q':
					...
					/*$q1 = 'SELECT `product_sku` FROM `#__virtuemart_products` WHERE `virtuemart_product_id`= "' . $value . '"';
					$this->_db->setQuery ($q1);
					$attr_virtuemart_product_id = $this->_db->loadResult ();*/				
				...
				case 'Z':
					...
					if ($category = $this->_db->loadObject ()) {
						...
						return JHTML::link (JRoute::_ ('index.php?option=com_virtuemart&view=category&virtuemart_category_id=' . $category->virtuemart_category_id), $thumb . ' ' . $category->category_showtitle, array('title' => $category->category_showtitle));
					}
					else return '';
				/* Child Group list
	...
	//vmdebug('customFieldDisplay $variantmods',$variantmods);
		...
		/// code for add custom product
		$db = JFactory::getDBO();
		if (!class_exists('VirtueMartCart'))
				require(JPATH_VM_SITE . DS . 'helpers' . DS . 'cart.php'); 
		$cart = VirtueMartCart::getCart();
		$product_custome_id = $product->customPrices[$product->virtuemart_product_id]['47'];
		$qty_temp_array = array();
		$total_qty = 0;
		$update_flag = false;
		// code end.
		
		 //print_r($_SESSION['temp_qty1']);
		...
		//foreach ($variantmods as $variant=> $selected) 
			...
			if ($selected) {
				...
				$query = 'SELECT `product_sku` from #__virtuemart_products  WHERE `virtuemart_product_id` ="' . (int)$productCustom->custom_value . '"';
				$db->setQuery($query);
				$product_sku_temp =  $db->loadResult();
				
				if($product_sku_temp != '')
				$_SESSION['temp_qty_sku'][$product_sku_temp] += (int)$product->quantity;
				
				$_SESSION['temp_qty'][$productCustom->custom_value] += (int)$product->quantity;
 				 
				
				if (!empty($productCustom) && !empty($cart)) { 
					foreach ($cart->products as $cprod_id => $c_prod_detail)
					{
					 	if (!array_key_exists($productCustom->custom_value, $cart->products))
						{ 
						//echo "if->".$productCustom->custom_value;
						$prod_ids = array($productCustom->custom_value);
						$success = false;
						$cart->add($prod_ids,$success);	
						}
						else
						{
							//echo 'fields-->'.$_SESSION['temp_qty'][$productCustom->custom_value].'<br>';
						//if($product->cart_item_id == $cprod_id)
						//echo "<pre>";print_r($productCustom);echo "</pre>"; die;
						   {
							 $cprod_id1 = $productCustom->custom_value;								
							 if($productCustom->custom_price == '')
							 $prod_custom_price = "0.00";
							 else
							 $prod_custom_price = $productCustom->custom_price;
							
							 $cart->products[$cprod_id1]->product_override_price = $prod_custom_price;
							 $cart->products[$cprod_id1]->quantity =  $_SESSION['temp_qty'][$productCustom->custom_value];  			
				...
				if (!empty($productCustom) && !empty($cart)) { 
					foreach ($cart->products as $cprod_id => $c_prod_detail)
					{	
					$pieces = explode(":", $cprod_id); 
					$pos = strpos($cprod_id, ':');
					if ($pos === false) 
					$pieces[0] = $cprod_id; 
					if($productCustom->custom_value != ''  &&  $pieces[2] != '')
					$add_c_pro_id = $productCustom->custom_value."::".$pieces[2].":47;";
					else
					{
					$add_c_pro_id = $cprod_id;
					/*$query = 'SELECT `virtuemart_customfield_id` from #__virtuemart_product_customfields  WHERE `custom_value` ="' . (int)$cprod_id . '" AND `virtuemart_custom_id` = "47"';
					$db->setQuery($query);
					$vm_customfield_id  =  $db->loadResult();
					$add_c_pro_id = $cprod_id."::".$vm_customfield_id.":47;";*/
					}
							if(($pieces[0] == $productCustom->custom_value))
							{
								if(($pieces[2] == $productCustom->virtuemart_customfield_id))	
								{
								$add_in_cart = false;
								//$cprod_id1 = $cprod_id; 
								$cprod_id1 = $add_c_pro_id;
								if($productCustom->custom_price == '')
								 $prod_custom_price = "0.00";
								 else
								 $prod_custom_price = $productCustom->custom_price;
								...
								// $cart->products[$cprod_id1]->product_override_price = $prod_custom_price;
								 $cart->products[$cprod_id1]->quantity =  $product->quantity;
								 //print_r($cart->products[$cprod_id1]);die;
								 }
							}
					 ...
					 if($add_in_cart)
					 {
					 $query = 'SELECT `virtuemart_product_id` from #__virtuemart_products  WHERE `virtuemart_product_id` ="' . (int)$productCustom->custom_value . '"';
					$db->setQuery($query);
					$id_available =  $db->loadResult();
					 
					 if($id_available){
					 $prod_ids = array($add_c_pro_id);
					 $success = false;
					 $cart->add($prod_ids,$success);					
					 }
