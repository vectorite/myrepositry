...
// 		vmTime('getCategories','getCategories');
	...
	*/
	 ...
	 *
	 */
	static function listMonths ($list_name, $selected = FALSE, $start = NULL, $end = NULL, $attr = '',$required = 0) {
		...
		//return JHTML::_ ('select.genericlist', $options, $list_name, '', 'value', 'text', $selected);
		return JHTML::_ ('select.genericlist', $options, $list_name, $attr, 'value', 'text', $selected,$required = 0);
   ...
   */
	 ...
	 * @return string HTML containing the <select />
	 */
	static public function renderZipcodeList ($zipcodeId = '0', $_prefix = '', $multiple = FALSE, $required = 0)
	 {
		$db = JFactory::getDBO();
		
		
			
		vmJsApi::JcountryZipcodeList ($zipcodeId);
		
		if ($multiple) {
			$attrs = 'multiple="multiple" size="12" name="' . $_prefix . 'virtuemart_zipcode_id[]" ';
		} else {
			$attrs = 'size="1"  name="' . $_prefix . 'virtuemart_zipcode_id" ';
		}
		...
		$query = 'SELECT * FROM `#__virtuemart_zipcode`';
		$db->setQuery($query);
		$zipcodes = $db->loadObjectList();
				
		$listHTML = '<select class="inputbox multiple" id="virtuemart_zipcode_id" ' . $attrs . '>';		
		foreach($zipcodes as $zipcode){
		if(in_array($zipcode->id,$zipcodeId))$sele = 'selected="selected"';
		$listHTML .='<option '.$sele.' value="'.$zipcode->id.'">'.$zipcode->zipcode.'</option>';
		$sele = '';
		}		
		$listHTML .='</select>';
		//echo "--->".$zipcodeId . $listHTML;die;
		return $listHTML;
	}
