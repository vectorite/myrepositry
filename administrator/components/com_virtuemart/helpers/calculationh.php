...
// 			if($this->_currencyDisplay_priceConfig['basePrice']) $this->_cartPrices[$cartproductkey]['subtotal'] = $product->prices['basePrice'] * $product->quantity;
		...
		// change tax if set in slaesrep order form 
		if(isset($_SESSION['item_discout_total']) != '')
		$this->_cartPrices['billTaxAmount'] = $_SESSION['item_discout_total'];
	...
	function gatherEffectingRulesForBill($entrypoint, $cartVendorId=1) {
		...
		foreach ($rules as $rule) {
			...
			$states = $this->_db->loadResultArray();
			 ...
			 if($this->_cart->BT['virtuemart_state_id'] != "")
			 $ss_state_id = $this->_cart->BT['virtuemart_state_id'];
			 
			 if($this->_cart->ST['virtuemart_state_id'] != "")
			 $ss_state_id = $this->_cart->ST['virtuemart_state_id'];
			 
			 if($this->_cart->BT['zip'] != "")
			 $ss_zip  = $this->_cart->BT['zip'];
			 
			 if($this->_cart->ST['zip'] != "")
			 $ss_zip  = $this->_cart->ST['zip'];
			 
			//echo "----ss_sate_id".$ss_state_id ."----ss_zip".$ss_zip;
			 if($ss_state_id == '10')
			{
			
			$q = 'SELECT `id` FROM #__virtuemart_zipcode WHERE `zipcode`="'.$ss_zip.'"';
			$this->_db->setQuery($q);
			$zipcode_id = $this->_db->loadResult();
			if($zipcode_id != "")
			{
			$q = 'SELECT `virtuemart_zipcode_id` FROM #__virtuemart_calc_zipcode WHERE `virtuemart_calc_id`="' . $rule["virtuemart_calc_id"] . '"';
			$this->_db->setQuery($q);
			$zipcodes = $this->_db->loadResultArray();
			}
			...
			} else if (!empty($states) && empty($zipcodes)) {
				$hitsDeliveryArea = $this->testRulePartEffecting($states, $this->_deliveryState);
				
			}else if($ss_state_id == '10')
			{
			 if (!empty($zipcodes)) {
				if($zipcode_id != "")
				$hitsDeliveryArea = $this->testRulePartEffecting($zipcodes, $zipcode_id);
				else
				$hitsDeliveryArea =0;				
			}}
			...
			//			}
				...
				if($ss_state_id == "10" && ($ss_zip == "" || $ss_zip == '11111')){
				$hitsDeliveryArea =0;		
				$hitsShopper = '';
				$testedRules = array();				
				}
