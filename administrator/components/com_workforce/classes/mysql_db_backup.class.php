...
Modifications for WorkForce by the Thinkery
*/
...
class wf_MySQL_DB_Backup
{
	...
	function Execute($task = MSX_STRING, $dname = '', $compress = false)
	{
		...
		if ($task == MSX_APPEND || $task == MSX_SAVE || $task == MSX_DOWNLOAD)
		{
			...
			{
                $v = new workforceAdmin; // to get version string
                $wfversion = $v->_getversion();
                
                $tmp_name = 'wfbackup-'.date($this->fname_format).'-v'.$wfversion;
	...
	function _GetTables()
	{
		...
		if (!sizeof($value))
		{
			$this->error = sprintf(JText::_('COM_WORKFORCE_NO_DB_TABLES'), implode(',',$this->tablesToInclude));
			return false;
		}
		return $value;
	}
	...
	function _OpenFile($fname, $task, $compress)
	{
		...
		if (!$fp)
		{
			$this->error = JText::_('COM_WORKFORCE_CANNOT_CREATE_OUTPUT');
			return false;
		}
		return $fp;
	}
	...
	function _DownloadFile($fname, $dname)
	{
		...
		if (!$fp)
		{
			$this->error = JText::_('COM_WORKFORCE_CANNOT_OPEN_TMP');
			return false;
		}
