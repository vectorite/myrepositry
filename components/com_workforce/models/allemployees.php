...
class workforceModelallemployees extends JModel
{
	var $_employees  = null;
	var $_where      = null;
	var $_total      = null;
	...
	function getData()
	{
		...
		if (empty($this->_employees))
		{
			...
			$sort		= $app->getUserStateFromRequest( $option.'.allemployees.filter_order', 'filter_order', 'd.ordering, e.ordering', 'cmd' );
			$order	    = $app->getUserStateFromRequest( $option.'.allemployees.filter_order_dir', 'filter_order_dir', 'ASC', 'word' );
	...
	function _buildContentWhere()
	{
        ...
        $this->_department = $app->getUserStateFromRequest( $option.'.allemployees.department'.$currid, 'department', '', 'int' );
        $this->_searchword = $app->getUserStateFromRequest( $option.'.allemployees.search'.$currid, 'search', '', 'string' );
        ...
        $where = array();
        if( $this->_department ) $where[]   = 'e.department = '.(int)$this->_department;
