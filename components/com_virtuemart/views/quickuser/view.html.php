...
class VirtuemartViewQuickuser extends VmView {
	...
	$this->_cuid = $this->_lists['current_id'] = 0;
	...
	if ((strpos($this->fTask, 'cart') || strpos($this->fTask, 'checkout')) && empty($virtuemart_userinfo_id)) {
	   ...
	   if (!$new && empty($userFields[$virtuemart_userinfo_id])) {
			$virtuemart_userinfo_id = 0;//$this->_model->getBTuserinfo_id();
...
*/
	...
	$this->_lists['canBlock'] = 0;//($this->_currentUser->authorize($comUserOption, 'block user')	&& ($this->_model->getId() != $this->_cuid)); // Can't block myself TODO I broke that, please retest if it is working again
	$this->_lists['canSetMailopt'] = 0;//$this->_currentUser->authorize('workflow', 'email_events');
	$this->_lists['block'] = 0;//JHTML::_('select.booleanlist', 'block', 'class="inputbox"', $this->_userDetails->JUser->get('block'), 'COM_VIRTUEMART_YES', 'COM_VIRTUEMART_NO');
	$this->_lists['sendEmail'] = 0;//JHTML::_('select.booleanlist', 'sendEmail', 'class="inputbox"', $this->_userDetails->JUser->get('sendEmail'), 'COM_VIRTUEMART_YES', 'COM_VIRTUEMART_NO');
