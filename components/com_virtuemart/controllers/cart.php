...
class VirtueMartControllerCart extends JController {
	...
	public function addJS() {
		...
		//$this->json->msg =$virtuemart_product_ids;
...
// 			vmdebug('setpayment $cart',$cart);
	...
	public function check_parend_child($prod_id=0) {
		/* Check for cart IDs */
		if (empty($prod_id))
		$prod_id = $_REQUEST['cart_virtuemart_product_id'];
		ob_clean();
		...
		//if($parent_prod_id != '')
		$custom_prod_id = $parent_prod_id."::".$exp_pro_id[2].":".$exp_pro_id[3];
		...
		if($parent_prod_id == '' )
		{
		$query = 'SELECT `custom_value` from #__virtuemart_product_customfields  WHERE `virtuemart_customfield_id` ="' . (int)$exp_pro_id[2] . '" AND `virtuemart_product_id`="'.(int)$exp_pro_id[0].'" AND `virtuemart_custom_id` ="47"';
		$db->setQuery($query);
		$custom_prod_id =  $db->loadResult();
		//if($custom_prod_id != '')		
		$custom_prod_id = $custom_prod_id."::".$exp_pro_id[2].":47;";
		}
