 ...
 * @version $Id: cart.php 6205 2012-07-03 13:10:28Z alatak $
 */
...
// 		$sessionCart->prices 						= $this->prices;
		...
		if($this->STsameAsBT)
		$sessionCart->ST = $sessionCart->BT;
		//print_r($sessionCart);die;		
// 		$pr = serialize($sessionCart->pricesUnformatted);
// 		vmdebug('$sessionCart',$sessionCart);
		$_SESSION['mera_cart'] = $sessionCart;
		if($_POST['ship_to_info_id'] != '')
		$_SESSION['mera_cart_shipto_id'] = $_POST['ship_to_info_id'];
...
// 			$product -> product_price_edate			= $tmpProduct -> product_price_edate;
			...
			if ($product) {
				...
				// code by RCA start. code for get qty in manually order.  
				if($quantityPost <= 0 || $quantityPost == "") $quantityPost = 1;
				
				if(isset($_SESSION['item_qty_relation'])){
					for($j=0;$j<count($_SESSION['item_qty_relation']);$j++)
					{
						if(trim($product->virtuemart_product_id) == trim($_SESSION['item_qty_relation'][$j]['item_id']))
						$quantityPost = $_SESSION['item_qty_relation'][$j]['item_qty'];
					}
				}
...
//====== add ipad condition rca chetnath ================
	...
	if($ipad) {
		...
		//$myquery1 = 'SELECT `virtuemart_product_id`,`virtuemart_customfield_id` from #__virtuemart_product_customfields  WHERE  `custom_value`="'.(int)$exp_pro_id[0].'" AND virtuemart_product_id="'.$my_child_product_id.'"';
		
		$myquery1 = 'SELECT `virtuemart_product_id`,`virtuemart_customfield_id` from #__virtuemart_product_customfields WHERE  `custom_value`="'.(int)$exp_pro_id[0].'"';
		
		$db->setQuery($myquery1);
		$myparent_prod_id =  $db->loadObjectList();
		...
		$myquery2 = 'SELECT `custom_value` from #__virtuemart_product_customfields  WHERE `virtuemart_customfield_id` ="' . (int)$mycustom_value_id . '" AND `virtuemart_product_id`="'.(int)$exp_pro_id[0].'"';
		$db->setQuery($myquery2);
		$mycustom_prod_id =  $db->loadResult();		
		$mycustom_prod_id = $mycustom_prod_id;
			
		if (array_key_exists($mycustom_prod_id, $this->products))
		 unset($this->products[$mycustom_prod_id]);

		//(string)$myids = $exp_pro_id[0];
	
		//if (array_key_exists($myids, $this->products))
		//	unset($this->products[$myids]);

	//====== end ipad condition rca chetnath ================
...
//====== original code =============
		...
		if(count($exp_pro_id) > 3 && end($exp_pro_id) == '47;')
		{
		if($exp_pro_id[2] != '' && $exp_pro_id[2] != '0')
		 $query = 'SELECT `virtuemart_product_id` from #__virtuemart_product_customfields  WHERE `virtuemart_customfield_id` ="' . (int)$exp_pro_id[2] . '" AND `custom_value`="'.(int)$exp_pro_id[0].'"';
		else
		 $query = 'SELECT `virtuemart_product_id` from #__virtuemart_product_customfields  WHERE  `custom_value`="'.(int)$exp_pro_id[0].'"';
	
		$db->setQuery($query);
		$parent_prod_id =  $db->loadResult();
		$parent_prod_id = $parent_prod_id."::".$exp_pro_id[2].":".$exp_pro_id[3];
		if (array_key_exists($parent_prod_id, $this->products))
		unset($this->products[$parent_prod_id]);
		...
		if(count($exp_pro_id) > 3 && end($exp_pro_id) == '47;')
		{ 
		  $custom_value_id = $this->products[$prod_id]->customPrices[$exp_pro_id[0]][47];
		
		$query = 'SELECT `custom_value` from #__virtuemart_product_customfields  WHERE `virtuemart_customfield_id` ="' . (int)$custom_value_id . '" AND `virtuemart_product_id`="'.(int)$exp_pro_id[0].'"';
		$db->setQuery($query);
		$custom_prod_id =  $db->loadResult();		
		$custom_prod_id = $custom_prod_id."::".$custom_value_id.":47;";
		if (array_key_exists($custom_prod_id, $this->products))
		unset($this->products[$custom_prod_id]);
		}
...
/**
	...
	function prepareAjaxData(){ 
		...
		if (!class_exists('CurrencyDisplay'))
			require(JPATH_VM_ADMINISTRATOR . DS . 'helpers' . DS . 'currencydisplay.php');
		$currency = CurrencyDisplay::getInstance();
		//print_r($_SESSION['temp_qty_sku']);
		$temp_qty_sku = $_SESSION['temp_qty_sku'];
		$_SESSION['temp_qty'] = '';
 		$_SESSION['temp_qty_sku']='';
...
// 			$this->data->products[$i]['prices'] = $this->prices[$priceKey]['subtotal_with_tax'];
		...
		$this->data->billTotal = $currency->priceDisplay( $this->pricesUnformatted['billTotal'] - $this->pricesUnformatted['billTaxAmount'] );
		//$this->data->billTotal = $currency->priceDisplay( $this->pricesUnformatted['billTotal'] );
