...
// 		JTable::addIncludePath(JPATH_VM_ADMINISTRATOR . DS . 'tables');
		...
		if(empty(self::$_cart)){
			...
			if (!empty($cartSession)) {
				...
				//$temp_qty     = '';
				//$temp_qty_sku = '';
				//$session->set('temp_qty',$temp_qty);
				//$session->set('temp_qty_sku',$temp_qty_sku);
...
// 		vmdebug('$sessionCart',$sessionCart);
        ...
        //$session->set('mera_cart',$sessionCart);
		...
		//$session->set('mera_cart_shipto_id',$ship_to_info_id);
...
// 			$product -> product_price_edate = $tmpProduct -> product_price_edate;
	...
	public function removeProductCart($prod_id=0) {
		...
		///start code for get attribute product_id and remove it in cart.
...
/**
	...
	function prepareAjaxData(){
		...
		$session =& JFactory::getSession();
        $temp_qty_sku = $session->get('temp_qty_sku');
		$session->set('temp_qty',$temp_qty1);
		$session->set('temp_qty_sku',$temp_qty_sku1);
       /* $temp_qty_sku = $session->get('temp_qty_sku'); //$_SESSION['temp_qty_sku'];
		$temp_qty1     = '';
		$temp_qty_sku1 = '';
