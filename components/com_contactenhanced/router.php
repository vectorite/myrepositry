...
function ContactenhancedParseRoute($segments)
{
	...
	if(is_object($categories)){
		...
		$categories = JCategories::getInstance('Contactenhanced')->get('root')->getChildren();
	} 
	$vars['catid'] = $id;
	$vars['id'] = $id;
	$found = 0;
	foreach($segments as $segment)
	{
		...
		if ($found == 0)
		{
			...
			{
				$db = JFactory::getDBO();
				$query = 'SELECT id FROM #__ce_details WHERE catid = '.$vars['catid'].' AND alias = '.$db->Quote($segment);
				$db->setQuery($query);
				$nid = $db->loadResult();
			} else {
				$nid = $segment;
