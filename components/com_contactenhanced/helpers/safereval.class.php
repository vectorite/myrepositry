...
class SaferEval extends JObject {
	var $source;
	var $errors 		= array();
	...
	var $allowedTokens 	= array (
			...
			'T_CASE',			// switch
			'T_CHARACTER',		// ?
		//	'T_CLOSE_TAG',		// Closing PHP script tag
			'T_COMMENT',			// comments
		...
		//	'T_ML_COMMENT',			// comments (PHP 4 only)
			...
			'T_NUM_STRING',			// ?
			'T_NEW',
		//	'T_OPEN_TAG',			// Open PHP script tag
			'T_OBJECT_OPERATOR',
                ...
                if (is_array($token)) {
					...
					switch ($id) {
						case('T_VARIABLE'): 
							...
							break;
					// Not checking for function names, so we can use any function in Joomla framework
					/*	case('T_STRING'):
