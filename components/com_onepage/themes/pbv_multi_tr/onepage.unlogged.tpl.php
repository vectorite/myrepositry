...
<?php 
	...
	if($_SESSION['checkvalue']==(int)1)
		$style = 'block';
	else
	 {
		$style = 'none';
		$_SESSION['checkvalue'] = '0';
	 }
?>

 <input type="checkbox" id="sachone" name="sa" value="adresaina"<?php if ($_SESSION['checkvalue']!='0') echo 'checked="checked"'; ?> onkeypress="showSA(sa, 'idsa');" onclick="javascript: showSA(sa, 'idsa');" autocomplete="off"/>

<!-- edit by rca chetnath pandey-->

<!--<input type="checkbox" id="sachone" name="sa" value="<?php echo $_SESSION['checkvalue']; ?>" onkeypress="showSA(sa, 'idsa');" onclick="javascript: showSA(sa, 'idsa');" autocomplete="off" />-->

<?php echo JText::_('COM_VIRTUEMART_USER_FORM_ADD_SHIPTO_LBL'); ?>
<div id="idsa" style="display:<?php echo $style; ?>;">
...
<input type="hidden" name="checkout_last_step" value="1" />
<input type="hidden" name="page" value="checkout.onepage" />
<input type="hidden" name="onepage" value="1" />
<input type="hidden" name="checkout_this_step[]" value="CHECK_OUT_GET_SHIPPING_METHOD" /><input type="hidden" name="checkout_this_step[]" value="CHECK_OUT_GET_PAYMENT_METHOD" /><input type="hidden" name="checkout_this_step[]" value="CHECK_OUT_GET_FINAL_CONFIRMATION" />
<!-- end of tricks -->
...
<input type="submit" class="buttonopc" value="<?php echo $VM_LANG->_('PHPSHOP_ORDER_CONFIRM_MNU') ?>" <?php echo $op_onclick?> id="confirmbtn" name="confirmbtn" />
</div>
<br style="clear: both;"/>
</div>
...
<script src="components/com_onepage/themes/pbv_multi_tr/jquery-1.8.3.min.js"></script>
<script>

jQuery(document).ready(function () {
  var source = jQuery("#email_field");
  var  dest = jQuery("#newmail_field");
 ...
 if(document.getElementById('adminForm').submit)
 {	
	  document.getElementById('email_field').value = document.getElementById('newmail_field').value;	  
 }
...
<script>
 ...
 jQuery(document).ready(function() {
        jQuery("#virtuemart_state_id").live('change', function() { //alert(this.options[this.selectedIndex].text);
    	  jQuery('#virtuemart_state_id option:selected').attr('selected', 'selected');
		 jQuery("#extra_field_2_field").val(this.options[this.selectedIndex].text);
...
if(document.getElementById('adminForm').submit)
 {	
    var no=document.getElementById("virtuemart_state_id");
	var option=no.options[no.selectedIndex].text;
	no.options[no.selectedIndex].text = document.getElementById("extra_field_2_field").value;
	
	
	//document.getElementById('virtuemart_state_id').options[document.getElementById('virtuemart_state_id').selectedIndex].text = document.getElementById('extra_field_2_field').value;
 }

/*$(document).ready(function () {
  var source1 = $("#virtuemart_state_id option:selected").val(); //$("#virtuemart_state_id option:selected");
  var  dest1 = $("#extra_field_2_field");
  
  $("#virtuemart_state_id").on('change',function() { alert(source1);
    dest1.val(source1.val());
  });
});*/

</script>
<style>
#newmail_div, #newmail_input, #extra_field_2_div, #extra_field_2_input
{
	display:none;
}
</style>
