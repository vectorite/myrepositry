	...
	class plgSearchVirtuemart extends JPlugin
	{
		...
		function onContentSearch($text, $phrase='', $ordering='', $areas=null) {
			...
			*/
			...
			$text = $db->Quote('%' . $db->getEscaped($text, true) . '%', false);
	/*
			$query = "SELECT DISTINCT CONCAT( a.product_name,' (',p.product_sku,')' ) AS title, p.product_sku, a.virtuemart_product_id , b.virtuemart_category_id ,   a.product_s_desc   AS text, b.category_name as section, p.created_on as created, '2' AS browsernav
...
//======== add by RCA chetnath =============
			...
			$query = "SELECT DISTINCT CONCAT( a.product_name,' (',p.product_sku,')' ) AS title, p.product_sku, p.product_params, a.virtuemart_product_id , b.virtuemart_category_id , a.product_name, a.product_s_desc   AS text, b.category_name as section, p.created_on as created, '2' AS browsernav, m1.file_url, pp.product_price, pp.product_override_price
					FROM `#__virtuemart_products_".VMLANG."` AS a
					JOIN #__virtuemart_products as p using (`virtuemart_product_id`)
					INNER JOIN `#__virtuemart_product_medias` as m2 ON p.`virtuemart_product_id` = m2.virtuemart_product_id 
				    INNER JOIN `#__virtuemart_medias` AS m1 ON m1.`virtuemart_media_id` = m2.`virtuemart_media_id`
					INNER JOIN `#__virtuemart_product_prices` AS pp ON pp.`virtuemart_product_id` = p.`virtuemart_product_id` 
					LEFT JOIN `#__virtuemart_product_categories` AS xref ON xref.`virtuemart_product_id` = a.`virtuemart_product_id`
					LEFT JOIN `#__virtuemart_categories_".VMLANG."` AS b ON b.`virtuemart_category_id` = xref.`virtuemart_category_id`"
			. ' WHERE ' . $where . ' and p.published=1 and b.virtuemart_category_id>0 and m1.file_is_product_image=1 GROUP BY p.`virtuemart_product_id`'
			. ' ORDER BY ' . $order ;
...
function plgSearchVirtuemart($text, $phrase='', $ordering='', $areas=null) {
	...
	$text = $db->Quote('%' . $db->getEscaped($text, true) . '%', false);
	$query = "SELECT DISTINCT CONCAT( a.product_name,' (',p.product_sku,')' ) AS title, p.product_sku, a.virtuemart_product_id , b.virtuemart_category_id ,   a.product_s_desc   AS text, b.category_name as section,
