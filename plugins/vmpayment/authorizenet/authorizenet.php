	...
	foreach ($this->methods as $method) {
		...
		if ($creditCards) {
		    $creditCardList = ($this->_renderCreditCardList($creditCards, $this->_cc_type, $method->virtuemart_paymentmethod_id, false));
					$_SESSION['cctype'] = $creditCardList;
		}	
		$sandbox_msg = "";
		if ($method->sandbox) {
...
/*========== Add by RCA ========== */
	...
	if(isset($_SESSION['cctype']))
		 $creditCardList = $_SESSION['cctype'];
	else
		 $creditCardList = '';

	if(isset($_SESSION['mycvv']))
		 $this->_cc_cvv = $_SESSION['mycvv'];
	else
		 $this->_cc_cvv = '';

	if(isset($_SESSION['myccno']))
		 $this->_cc_number = $_SESSION['myccno'];
	else
		 $this->_cc_number = '';

	if(isset($_SESSION['mymonth']))
		 $this->_cc_expire_month = $_SESSION['mymonth'];
	else
		$this->_cc_expire_month = '';
		
	 if(isset($_SESSION['myyear']))
		  $this->_cc_expire_year = $_SESSION['myyear'];
	else
		$this->_cc_expire_year = '';

/*========== Edit by RCA chetnath ========== */
		...
		$html .= '<br /><span class="vmpayment_cardinfo">' . JText::_('VMPAYMENT_AUTHORIZENET_COMPLETE_FORM') . $sandbox_msg . '
		    ...
		    <!--<tr valign="top">
				...
				<tr valign="top">
		        <td>
				<div title="' . JText::_('VMPAYMENT_AUTHORIZENET_CCTYPE') . '*" id="virtuemart_cc_type_2_input" class="formField">
				<span class="formLabel " style="visibility:hidden;width:180px!important;" id="cc_number_' . $method->virtuemart_paymentmethod_id . '_div">Card Type</span>				
				<div class="before_select"></div>
				<div style="" class="middle_select">
				<div style="" class="after_select">&nbsp;</div>
				' . $creditCardList .
				'
				
				</div></div>
				<div title="' . JText::_('VMPAYMENT_AUTHORIZENET_CCNUM') . '*" id="cc_number_' . $method->virtuemart_paymentmethod_id . '_input" class="formField">

				<span class="formLabel " style="" id="cc_number_' . $method->virtuemart_paymentmethod_id . '_div">Credit Card Number:*</span>
				<div class="before_input"></div>
				<div class="middle_input">

		        <input style="width: 86%;" type="text" maxlength="32" class="required" size="30" onfocus="inputclear(this)" aria-required="true" required="required" id="cc_number_' . $method->virtuemart_paymentmethod_id . '_field" name="cc_number_' . $method->virtuemart_paymentmethod_id . '" value="' . $this->_cc_number . '"    autocomplete="off"   onchange="ccError=razCCerror(' . $method->virtuemart_paymentmethod_id . ');
	...
	if (!ccError) {
	    this.value=\'\';}" />
				...
				<input type="hidden" value="'. JText::_('VMPAYMENT_AUTHORIZENET_CCNUM') .'" name="savedtitle" id="saved_cc_number_' . $method->virtuemart_paymentmethod_id . '_field">

<div class="after_input">&nbsp;</div>
		   </div></div></td>
		    </tr>
		   		<!--<tr valign="top">
		        <td nowrap width="10%" align="left">
			...
			<div title="' . JText::_('VMPAYMENT_AUTHORIZENET_CVV2') . '*" id="cc_cvv_2_input" class="formField">
			<!--<span class="formLabel" style="" id="cc_cvv_2_div">Missing</span>-->
			<div class="before_input"></div>
			<div class="middle_input"  style="margin-top: 12px;">	
			
			
		    <input type="text"  id="cc_cvv_'. $method->virtuemart_paymentmethod_id . '_field" name="cc_cvv_'. $method->virtuemart_paymentmethod_id . '" maxlength="4" size="10" value="' . $this->_cc_cvv .'"  onfocus="inputclear(this)" aria-required="true" required="required" class="required"  />

			<input type="hidden" value="'.JText::_('VMPAYMENT_AUTHORIZENET_CVV2') .'*" name="savedtitle" id="saved_cc_cvv_2_field">
			<label class="userfields" id="label_cc_cvv_2_field" for="cc_cvv_2_field">' . JText::_('VMPAYMENT_AUTHORIZENET_CVV2') . '*</label>
			<div class="hasTip" title="' . JText::_('VMPAYMENT_AUTHORIZENET_WHATISCVV') . '::' . JText::sprintf("VMPAYMENT_AUTHORIZENET_WHATISCVV_TOOLTIP", $cvv_images) . ' ">' .
			JText::_('VMPAYMENT_AUTHORIZENET_WHATISCVV') . '
			</div>
			<div class="after_input">&nbsp;</div>
			</div>
			</div>
			<div style="width:200px;float:left;height:60px;">
			<span class="formLabel" align="left">' . JText::_('VMPAYMENT_AUTHORIZENET_EXDATE') . '</span>
			';
		//$html .= shopfunctions::listMonths('cc_expire_month_' . $method->virtuemart_paymentmethod_id, $this->_cc_expire_month);
		
		// ======== add by chetnath pandey RCA ===========

		$html .= shopfunctions::listMonths('cc_expire_month_' . $method->virtuemart_paymentmethod_id, $this->_cc_expire_month, null, null, "onfocus=\"inputclear(this)\" aria-required=\"true\" required=\"required\" class=\"cc_expire_month_2 required\" ");
		// Original line
		//$html .= shopfunctions::listMonths('cc_expire_month_' . $method->virtuemart_paymentmethod_id, $this->_cc_expire_month, null, null, "onfocus=\"inputclear(this)\" aria-required=\"true\" required=\"required\" class=\"cc_expire_month_2 required invalid\" ");

		$html .= " / ";

		// ======== add by chetnath pandey RCA ===========

		$html .= shopfunctions::listYears('cc_expire_year_' . $method->virtuemart_paymentmethod_id, $this->_cc_expire_year, null, null, "onchange=\"var month = document.getElementById('cc_expire_month_'.$method->virtuemart_paymentmethod_id); if(!CreditCardisExpiryDate(month.value,this.value, '.$method->virtuemart_paymentmethod_id.')){this.value='';month.value='';}\",onfocus=\"inputclear(this)\" aria-required=\"true\" required=\"required\" class=\"cc_expire_year_2 required\" ");
		// original line
		//$html .= shopfunctions::listYears('cc_expire_year_' . $method->virtuemart_paymentmethod_id, $this->_cc_expire_year, null, null, "onchange=\"var month = document.getElementById('cc_expire_month_'.$method->virtuemart_paymentmethod_id); if(!CreditCardisExpiryDate(month.value,this.value, '.$method->virtuemart_paymentmethod_id.')){this.value='';month.value='';}\",onfocus=\"inputclear(this)\" aria-required=\"true\" required=\"required\" class=\"cc_expire_year_2 required invalid\" ");
